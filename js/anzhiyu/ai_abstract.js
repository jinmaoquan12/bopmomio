(function () {
  ...
  async function readAloud() {
    ...
    try {
      const response = await fetch(`https://api.openai.com/?${requestParams}`, requestOptions);
  ...
  async function aiAbstractOpenAI(num) {
    ...
    const requestOptions = {
        ...
        headers: {
            "Content-Type": "application/json",
            "Authorization": AIKey, // Replace with your OpenAI API key
        },
    ...
    try {
        ...
        if (summary) {
            ...
            startAI("Summary generation failed. Please try again later.");
        }
    } catch (error) {
        console.error(error);
        startAI("Error: Unable to generate summary. Please try again later.");
    }
  }
